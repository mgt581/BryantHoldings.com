...
const initLeadForms = () => {
  document.querySelectorAll('[data-lead-form]').forEach((form) => {
    form.addEventListener('submit', async (event) => {
      ...
      Object.assign(payload, window.LeadGen ? window.LeadGen.getAttribution() : {});
      ...
      window.LeadGen?.trackEvent('lead_form_submit_attempt', {
      ...
      try {
        ...
        if (!response.ok || !result.ok) throw new Error(result.error || 'Your enquiry could not be sent online.');
        ...
        if (status) status.textContent = 'We could not confirm online delivery. Please call 07843 969254 if your enquiry is urgent.';
      } finally {
        if (button) {
